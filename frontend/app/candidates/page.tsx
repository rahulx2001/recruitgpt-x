...
function CandidatesView() {
  ...
  const rankById = React.useMemo(() => {
    const sorted = [...basePool].sort((a, b) => b.matchScore - a.matchScore);
    return new Map(sorted.map((c, i) => [c.id, i + 1]));
  }, [basePool]);

  const poolAvgs = React.useMemo(() => {
    if (!basePool.length) {
    ...
    return {
      match: Math.round(
        basePool.reduce((s, c) => s + c.matchScore, 0) / n
      ),
      skills: Math.round(
        basePool.reduce((s, c) => s + c.skillsMatch, 0) / n
      ),
      experience: Math.round(
        basePool.reduce((s, c) => s + c.experienceMatch, 0) / n
      ),
      github: Math.round(
        basePool.reduce((s, c) => s + c.githubMatch, 0) / n
      ),
    };
  }, [basePool]);
