...
def main() -> int:
    ...
    s = report.get("synthetic_proxy_eval", {})
    ...
    print(f"Self-consistency proxy — NDCG@10={m.get('ndcg_10')} (NOT hidden GT)")
    if b:
        print(f"Behavioral proxy — NDCG@10={b.get('ndcg_10')} MAP={b.get('map')}")
    if s:
        m50 = s.get("metrics_at_0.5") or s.get("metrics", {})
        print(
            f"Synthetic proxy labels — NDCG@10={m50.get('ndcg_10')} p@10={m50.get('p_at_10')} "
            f"n={s.get('n_labeled')} relevant@0.5={s.get('n_relevant_threshold_0.5')} "
            "(auto-generated, NOT human GT)"
        )
    if best:
        print(f"Best ablation (behavioral proxy): {best.get('name')} NDCG@10={best.get('ndcg_10')}")
    sa = report.get("weight_ablation_on_synthetic_proxy") or {}
    if sa and s.get("best_preset_by_ndcg10"):
        bs = s["best_preset_by_ndcg10"]
        print(f"Best ablation (synthetic proxy): {bs.get('name')} NDCG@10={bs.get('ndcg_10')}")
